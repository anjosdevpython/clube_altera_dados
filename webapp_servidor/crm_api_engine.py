...
class CrmApiEngine:
    ...
    Otimizado para execução em Servidor Interno (Windows Server / Servidor AD / IIS / Waitress).
    """
    ...
    def alterar_cliente_api(self, cpf: str, novo_email: Optional[str] = None, nova_senha: Optional[str] = None) -> Tuple[bool, int, str]:
        """
        Busca e altera os dados do cliente (Email/Senha) no CRM Bnex via HTTP.
        """
        t0 = time.time()
        if not self.session:
